...
public class RandomGen implements RandomGenerator {
  ...
  public static final String DUPLICATE_NUMBER_ERROR_MESSAGE = "randomNums cannot contain duplicates";

  /**
   * Allocates a new {@code RandomGen} with the given parameters
   *
   * @param randomNums    The list of numbers to be random selected from
   * @param probabilities The probability of the corresponding number in {@code randomNums} being
   *                      chosen
  ...
  /**
   * Returns one of the randomNums. When this method is called multiple times over a long period, it
   * should return the numbers roughly with the initialized probabilities.
   *
   * @implNote if no number is selected on an initial function call then the function will be called
   * again. This means a number will always be returned
   */
  ...
  public int nextNum() throws Exception {
    ...
    for (int i = 0; i < getRandomNums().length; i++) {
      ...
      boolean selected = randomFloat < upperBound
          && randomFloat >= lowerBound; // the probability this is true is equal to p[i]
      if (selected) {
